...
namespace Roots\Sage\HeroRender;
	...
	// Get hero content
	function phyt_get_hero_content( $post_id = null ) {
	...
	function phyt_has_hero( $post_id = null ) {
		...
		// Get hero content
		$hero = phyt_get_hero_content( $post_id );
	...
	function phyt_get_hero( $post_id = null ) {
		...
		?><div class="hero <?php echo ( empty( $hero['img'] ) ? 'bg-muted' : 'bg-dark text-shadow' ); ?> margin-bottom" <?php if ( !empty( $hero['img'] ) ) { echo 'style="' . ( empty( $hero['min_height'] ) ? '' : 'min-height: ' . $hero['min_height'] . '; ' ) . 'background-image: ' . ( empty( $hero['overlay'] ) ? '' : 'linear-gradient( '.$value_type .'(' . $overlay . ', ' . $transparency . '), '.$value_type .'(' . $overlay . ', ' . $transparency . ') ),' ) . ' url(' . $hero['img'][0] . ');"'; } ?>>
